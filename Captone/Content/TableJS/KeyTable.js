...
function KeyTable ( oInit )
{
	...
	function _fnCellFromCoords( x, y )
	{
		...
		{
			var oSettings = _oDatatable.fnSettings();
			if ( oSettings && typeof oSettings.aoData[ oSettings.aiDisplay[ y ] ] != 'undefined' )
			{
	...
	function _fnInit( oInit, that )
	{
		...
		_nBody = oInit.table && oInit.table.getElementsByTagName('tbody')[0];
		 ...
		 */
		if (jQuery.browser && jQuery.browser.mozilla || jQuery.browser && jQuery.browser.opera)
		{
		...
		if ( _oDatatable )
		{
		    jQuery('tbody td', _oDatatable.fnSettings() !== null && _oDatatable.fnSettings().nTable).on( 'click', _fnClick );
		}
		else
		{
